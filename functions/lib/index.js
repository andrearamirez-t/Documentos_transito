...
function getGeminiClient() {
    ...
    if (!apiKey)
        throw new Error("GEMINI_API_KEY no configurada en Firebase secrets.");
    if (!geminiClient) {
...
function validateDocumentPayload(documentType, extractedData) {
    const rules = REQUIRED_FIELDS[documentType];
    if (!rules)
        return { valid: false, missingFields: [{ field: "documentType", label: `Tipo desconocido: "${documentType}"` }] };
...
app.post("/api/documents/analyze", async (req, res) => {
    ...
    if (!base64)
        return res.status(400).json({ error: "No se proporcionó data en base64." });
    try {
...
- Pay attention to handwritten data in margins or corners.`;
        ...
        const newRecords = results.filter((r) => r !== null);
        ...
        const isKeyError = ((_a = error.message) === null || _a === void 0 ? void 0 : _a.includes("API_KEY")) || ((_b = error.message) === null || _b === void 0 ? void 0 : _b.includes("PERMISSION"));
...
app.post("/api/documents/sync/:id", async (req, res) => {
    ...
    const syncLog = { timestamp: new Date().toISOString(), url: destinationUrl, method: "POST", requestPayload: apiPayload };
    try {
...
app.post("/api/validate", (req, res) => {
    ...
    if (!documentType || !extractedData)
        return res.status(400).json({ status: "error", message: "Se requieren documentType y extractedData." });
    const { valid, missingFields } = validateDocumentPayload(documentType, extractedData);
    if (!valid)
        return res.status(422).json({ status: "validation_error", documentType, valid: false, missingCount: missingFields.length, missingFields });
...
app.post("/api/external-system-mock", async (req, res) => {
    ...
    if (documentType === "cedula") {
        clientName = extractedData.nombreCompleto;
        clientDocId = extractedData.numeroDocumento;
    }
    else if (documentType === "factura") {
        clientName = extractedData.compradorNombre;
        clientDocId = extractedData.compradorIdentificacion;
        vehicleChasis = extractedData.vehiculoChasis;
        vehicleMotor = extractedData.vehiculoMotor;
    }
    else if (documentType === "runt") {
        clientName = extractedData.propietarioNombre;
        clientDocId = extractedData.propietarioIdentificacion;
        vehicleChasis = extractedData.vehiculoChasis;
        vehicleMotor = extractedData.vehiculoMotor;
    }
        ...
        if (marca || linea)
            clientName = `Vehículo ${marca} ${linea}`.trim();
    }
    else if (documentType === "poder") {
        clientName = extractedData.otorganteNombre;
        clientDocId = extractedData.otorganteIdentificacion;
    }
